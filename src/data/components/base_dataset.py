...
class BaseDataset(Dataset):
    ...
    def process_data(self) -> None:
        ...
        if len(self.scenario_list.keys()) > 0:
            files_per_airport = max(
                min(len(f) for _, f in self.scenario_list.items()), 500000)
            balanced_list = []
    ...
    def process_seq(
        ...
        for _, agent_id in enumerate(unique_agents):
            ...
            # Interpolated mask
            mask = agent_seq[:, G.RAW_IDX.Interp] == '[ORG]'  # Not interpolated -->     Valid
            agent_seq[mask, G.RAW_IDX.Interp] = 1.0  # Interpolated --> Not valid
